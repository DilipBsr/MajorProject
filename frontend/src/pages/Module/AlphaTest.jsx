...
const BASE_URL = "http://localhost:4999"; // Update with your Flask API URL
...
const AlphaTest = () => {
  ...
  const checkAlphabetMatch = useCallback((label, confidence) => {
    ...
    setTimeout(() => {
      ...
      if (label === currentAlphabet && confidence >= 50) {
        ...
        setShowPopup(true);  // Show the popup when match is found
      }
    }, 300)
  ...
  useEffect(() => {
    ...
    if (isCameraOn && !showPopup) {
      interval = setInterval(captureFrame, 300); // Capture frame every 500ms
    }
    ...
    <>
      <div className="flex flex-col items-center bg-orange-100 min-h-screen">
        ...
        <div className="flex flex-wrap lg:w-4/6 justify-center p-5 gap-3 m-2">
          {ALPHABETS.map((letter, idx) => (
            <span
              key={idx}
              onClick={() => !visited[idx] && setCurrentAlphabetIndex(idx)}  // Click to change the current letter
      ...
      {showAlert && (
        <Alert message={"Already Matched"}
          onClose={() => {
            setShowAlert(false);
            setVisited(new Array(26).fill(false));
          }} />
      )}
    </>
  );
